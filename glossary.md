...
An instance of TREE-SEARCH or GRAPH-SEARCH in which a node is selected based on an evaluation function.
...
The maximum number of successors of any node in a tree.
...
A search strategy in which nodes at a certain depth (called limit and denoted by l) are treated as if they have no children.
...
**First In First Out (FIFO) queue** - a queue variant which pops the oldest element of the queue.
...
A **genetic algorithm** (GA) is a variant of stochastic beam search in which successor states are generated by combining two parent states rather than by modfying a single state.
...
Objectives that an agent is trying to achieve.

## goal clauses
## goal formulation

The first step in problem solving in which goals are determined.
...
A search strategy in which nodes closest to the goal are expanded first.
...
A search strategy in which depth limited search is applied iteratively over limit from 0 to maximum length.

## iterative expansion
## iterative-deepening A*

An iterative deepening search in which, at each iteration, the cutoff value is the smallest f-cost of any node that exceeded the cutoff on the previous iteration.
...
**Last In First Out (LIFO) queue** - a queue variant which pops the newest element of the queue. It is also called as stack.
...
Search strategies which operate on a single current node (rather than multiple paths) and generally move only to neighbors of that node.
